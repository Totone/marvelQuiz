...
/** 
 * TOTONE => https://www.github.com/Totone <= Say hi, thanks or anything else to make me feel happy !
 * 
...
const storage = () => {
   ...
   */
  const getDataFromLS = (id, parse = true) => checkDataInLS(id, false, parse);
  
  /**
   * Checks if data corresponding to id is in LS & returns the corresponding boolean.
   * @param {string} id Key identifier
   */
  const isDataInLS = (id) => checkDataInLS(id, true);
   ...
   */
  const setDataToLS = (id, data, stringify = true) => {
...
export const checkStoredDataAge = storage().checkStoredDataAge;
export const getDataFromLS = storage().getDataFromLS;
export const isDataInLS = storage().isDataInLS;
export const setDataToLS = storage().setDataToLS;